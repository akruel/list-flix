...
function getPublicPathsFromRouteTree(repoRoot: string): string[] {
  ...
  const fullPathsBlock = source.match(
    /fullPaths:\s*([\s\S]*?)\n\s*fileRoutesByTo:/,
  );
...
test("JOURNEY_CONTRACT_SCENARIOS: required scenarios must exist and cannot be skipped", () => {
  ...
  const specSources = specFiles.map((filePath) =>
  ...
  const requiredScenarioIds = Array.from(
    new Set(
      journeyCoverageManifest.flatMap((journey) => journey.requiredScenarioIds),
    ),
  ).sort();

  const missingScenarioIds = requiredScenarioIds.filter(
    (scenarioId) => !declaredScenarioIds.has(scenarioId),
  );
  const requiredButSkipped = requiredScenarioIds.filter((scenarioId) =>
...
test("JOURNEY_CONTRACT_SCENARIO_COVERAGE: all SCENARIO_IDS must belong to journeys and vice-versa", () => {
  ...
  const missingInJourneys = declaredScenarioIds.filter(
    (scenarioId) => !journeyScenarioIds.includes(scenarioId),
  );
...
test("JOURNEY_CONTRACT_PATHS: all public paths and NOT_FOUND must belong to journeys", () => {
  const repoRoot = process.cwd();
  const publicPaths = getPublicPathsFromRouteTree(repoRoot)
  ...
  const missingPublicPaths = publicPaths.filter(
    (routePath) => !touchedPaths.includes(routePath),
  );
  const unknownTouchedPaths = touchedPaths.filter(
    (routePath) => !publicPaths.includes(routePath),
  );
