...
const routeCoverageManifest: RouteCoverageEntry[] = [
  ...
  },
  {
    routePath: "/details/$type/$id",
    routeFile: "src/routes/_protected/details/$type/$id.tsx",
    scenarioId: SCENARIO_IDS.DETAILS_ADD_TO_LIST,
    expectation: "render",
  },
  {
    routePath: "/details/$type/$id",
...
test("ROUTE_MANIFEST_FILES: every route file in manifest exists", () => {
  ...
  const missingRouteFiles = routeCoverageManifest
    ...
    .filter(
      (routeFile, index, allFiles) => allFiles.indexOf(routeFile) === index,
    )
...
test("ROUTE_PATH_CONTRACT: every public routeTree path and NOT_FOUND are represented in manifest", () => {
  ...
  const missingPublicPaths = publicPaths.filter(
    (routePath) => !manifestPaths.includes(routePath),
  );

  expect(
    routeCoverageManifest.some((entry) => entry.routePath === "NOT_FOUND"),
  ).toBe(true);
...
test("ROUTE_SCENARIO_MAPPING: route specs, manifest and SCENARIO_IDS stay in sync", () => {
  ...
  const manifestScenarioIds = new Set<string>(
    routeCoverageManifest.map((entry) => entry.scenarioId),
  );
