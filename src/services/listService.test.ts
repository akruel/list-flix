...
describe("listService", () => {
  ...
  it.each(roleCases)(
    "maps current user in getLists for $caseName",
    async ({ role }) => {
  ...
  it.each([
    ...
    "uses metadata fallback for $caseName in createList",
  ...
  it("throws when createList fails on insert", async () => {
    ...
    await expect(listService.createList("Shared")).rejects.toThrow(
      "insert failed",
    );
  });
  ...
  it("throws when getListDetails fails loading list", async () => {
    ...
    mockedSupabase.from.mockImplementation((table: string) => {
      ...
      return {
        select: vi
          .fn()
          .mockReturnValue(createThenableBuilder({ data: [], error: null })),
      };
    });

    await expect(listService.getListDetails("list-1")).rejects.toThrow(
      "list failed",
    );
  });
  ...
  it("throws in joinList when there is no authenticated user", async () => {
    ...
    await expect(listService.joinList("list-1", "Alice")).rejects.toThrow(
      "User not authenticated",
    );
  });
  ...
  it.each(joinRoleCases)(
    "inserts new membership for $caseName",
    async ({ role }) => {
      mockedSupabase.auth.getUser.mockResolvedValue({
        data: {
          user: { id: "user-1" },
      ...
      const insert = vi
        .fn()
        .mockImplementation((payload: Array<Record<string, unknown>>) => {
  ...
  it("returns list name from RPC", async () => {
    ...
    expect(mockedSupabase.rpc).toHaveBeenCalledWith("get_list_name", {
      list_id: "list-1",
    });
  });
  ...
  it("throws when getListName RPC fails", async () => {
    ...
    await expect(listService.getListName("list-1")).rejects.toThrow(
      "rpc failed",
    );
  });
  ...
  it.each([
    ...
    await expect(
      listService.getListsContainingContent(10, contentType),
    ).resolves.toEqual({
      "list-a": "item-a",
      "list-b": "item-b",
    });
  ...
  it("throws when getListsContainingContent fails", async () => {
    ...
    await expect(
      listService.getListsContainingContent(10, "movie"),
  ...
  it.each([
    ...
    "throws for $caseName",
    async ({ run, table, method, expectedMessage }) => {
      const builder = createThenableBuilder({
        data: null,
  ...
  it("updates list name and throws on update failure", async () => {
    ...
    await expect(
      listService.updateList("list-1", "New"),
