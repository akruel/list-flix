...
describe("migrationService", () => {
  ...
  it.each([
    ...
    await expect(
      migrationService.migrateAnonymousUserData(oldUserId, newUserId),
  ...
  it("runs migration and shows success toast", async () => {
    ...
    expect(toastMocks.success).toHaveBeenCalledWith(
      "Suas listas e dados foram migrados com sucesso!",
      {
  ...
  it.each([
    ...
    },
    {
      caseName: "rpc throws",
      setup: () =>
        mockedSupabase.rpc.mockRejectedValue(new Error("rpc crashed")),
    },
  ])("handles failures for $caseName", async ({ setup }) => {
    setup();

    await expect(
      migrationService.migrateAnonymousUserData("old-1", "new-1"),
    ).resolves.toBeUndefined();
