...
describe.sequential("RLS: list_items policies", () => {
  it("allows owner and editor to manage items", async () => {
    ...
    try {
      ...
      expect(await hasItemWithContentId(fixture.listId, ownerContent)).toBe(
        true,
      );

      const editorContent = uniqueContentId();
      const editorInsert = await fixture.editor.client
        .from("list_items")
        .insert({
          list_id: fixture.listId,
      ...
      expect(await hasItemWithContentId(fixture.listId, editorContent)).toBe(
        true,
      );
  ...
  it("prevents viewers from managing items", async () => {
    ...
    try {
      ...
      const viewerInsert = await fixture.viewer.client
        .from("list_items")
        .insert({
          list_id: fixture.listId,
  ...
  it("allows list members to read items and blocks non-members", async () => {
    ...
    try {
      ...
      const ownerRead = await listItemsByUser(
        fixture.owner.client,
        fixture.listId,
      );
      const editorRead = await listItemsByUser(
        fixture.editor.client,
        fixture.listId,
      );
      const viewerRead = await listItemsByUser(
        fixture.viewer.client,
        fixture.listId,
      );
      const outsiderRead = await listItemsByUser(
        fixture.outsider.client,
        fixture.listId,
      );
