...
describe.sequential("RLS: list_members policies", () => {
  it("allows joining as viewer/editor and blocks joining as owner", async () => {
    ...
    try {
      const joinAsViewer = await fixture.outsider.client
        .from("list_members")
        .insert({
      ...
      expect(await getMemberRole(fixture.listId, fixture.outsider.id)).toBe(
        "viewer",
      );
      ...
      const joinAsOwner = await outsiderOwner.client
        .from("list_members")
        .insert({
          list_id: fixture.listId,
  ...
  it("allows owner to remove editor/viewer but not owner", async () => {
    ...
    try {
      ...
      expect(await getMemberRole(fixture.listId, fixture.owner.id)).toBe(
        "owner",
      );
    } finally {
      await teardownFixture(fixture);
  ...
  it("allows owner to update roles between viewer and editor", async () => {
    ...
    try {
      ...
      expect(await getMemberRole(fixture.listId, fixture.viewer.id)).toBe(
        "editor",
      );
  ...
  it("prevents non-owner users from managing members", async () => {
    ...
    try {
      ...
      expect(await getMemberRole(fixture.listId, fixture.editor.id)).toBe(
        "editor",
      );
